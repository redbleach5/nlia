/**
 * VRM layout — normalize by head bone, then ground feet.
 * Ported from LIA-main vrm/layout.ts (no avatar-config dependency).
 */

import * as THREE from "three";
import type { VRM } from "@pixiv/three-vrm";

/** Target head bone height (m) for portrait camera framing. */
...
function readHeadHips(vrm: VRM): { headY: number; hipsY: number } {
  ...
  if (humanoid) {
    const head =
      humanoid.getNormalizedBoneNode("head" as never) ??
      humanoid.getRawBoneNode("head" as never);
    const hips =
      humanoid.getNormalizedBoneNode("hips" as never) ??
...
/** Shift model so bbox min Y = 0 (+ optional offset). */
 ...
 * Call AFTER rotateVRM0; re-ground after arm pose if needed.
 */
export function applyVrmLayout(
  vrm: VRM,
  opts: { userScale?: number; yOffset?: number } = {},
): VrmLayoutMetrics {
  const userScale = Number.isFinite(opts.userScale) ? (opts.userScale as number) : 1;
  const yOffset = Number.isFinite(opts.yOffset) ? (opts.yOffset as number) : 0;
